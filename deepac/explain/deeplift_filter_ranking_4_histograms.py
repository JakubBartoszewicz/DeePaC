...
if args.mode == "original":
	...
	for filter, scores in filter_scores.items():
		...
		if mean_scores_wo_zeros >= 0:
			...
			metrics = [mean_scores_wo_zeros, mean_scores_w_zeros, min(mean_scores_wo_zeros), max(mean_scores_wo_zeros), TP/(TP+FN), TN/(FP+TN), (TP+TN)/(TP+FP+FN+TN), TP, FP, TN, FN, float(len(scores))/float(len(scores_all_reads))]
