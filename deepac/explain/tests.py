...
class ExplainTester:
    ...
    def test_maxact(self):
        ...
        args = Namespace(model=self.model, test_data=self.test_data, nonpatho_test=self.neg_fasta,
                         ...
                         n_cpus=self.n_cpus, do_lstm=False, inter_layer=1, chunk_size=500, save_activs_and_maxact=False,
                         save_activs_only=False)
        get_maxact(args)
    ...
    def test_fcontribs(self):
        ...
        assert (os.path.isfile(os.path.join(self.outpath, "fcontribs0", "fasta",
                                            "sample_val_data_motifs_filter_1.fasta"))), "Fcontribs for last layer" \
                                                                                        " failed."
        assert (os.path.isfile(os.path.join(self.outpath, "fcontribs0", "filter_scores",
                                            "sample_val_data_rel_filter_1.csv"))), "Fcontribs for last layer failed."
    ...
    def test_franking(self):
        ...
        args = Namespace(mode="original", scores_dir=scores_dir, true_label=y, pred_label=y_pred,
                         out_dir=os.path.join(self.outpath, "franking"), target_class=self.target_class)
