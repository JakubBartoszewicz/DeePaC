...
def generate_multiclass_sample_data(gc_per_class=(0.3, 0.7, 0.1, 0.9), n_train=1024, n_val=1024):
    ...
    for i in range(n_classes):
        ...
        generate_species_csv(i, class_val, 2, f"deepac-tests-{i}-reads.csv")
...
def generate_species_csv(class_id, n_reads, n_species, filename):
    ...
    with open(os.path.join("deepac-tests", filename), 'w') as outfile:
        for s in range(n_species):
            outfile.write(f"species_{class_id}_{s};{int(n_reads/n_species)}\n")
