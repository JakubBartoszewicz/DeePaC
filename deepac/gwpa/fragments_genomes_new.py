...
if args.shift > args.read_len:
	print("Attention: Shift (" + str(args.shift) + ") is larger than read length (" + str(args.read_len) + ")!")
	...
	if file.endswith(".fasta") or file.endswith(".fna"):
		...
		for instance in instances:
			...
			for i in range(num_reads):
				...
				new_record = SeqRecord(seq_pad[start:end], id = instance.id + ":" + str(start-pad_left) + ".." + str(end-pad_left), name = instance.id + ":" + str(start-pad_left) + ".." + str(end-pad_left), description = "")
				fragmented_genomes.append(new_record)

		SeqIO.write(fragmented_genomes, args.out_dir + "/" + os.path.splitext(file)[0] + "_fragmented_genomes.fasta", "fasta")
