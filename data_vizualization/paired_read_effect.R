...
# diff in performane
plot_paired_read_eff <- function(ylab,y,nn_type,table=FALSE, ylim=c(NA,1), read_lengths=c(50,100,150,200,250)){
...
plot <- ggplot(data = data_temp_paired,
  ...
  if(table){
  data_temp <- data_temp[,c("fragment_length",y,"subread_length","pred_type","set")]
...
# overview plots
cnn_overview <- plot_paired_read_eff(ylab = "accuracy",y = "acc",ylim = c(0.5,0.9),table = FALSE,nn_type = "cnn") 
cnn_overview

lstm_overview <- plot_paired_read_eff(ylab = "accuracy",y = "acc",ylim = c(0.5,0.9),table = FALSE,nn_type = "lstm") 
lstm_overview

# plot all metrics
for(type in c("cnn","lstm")){
    for(read_length in c(50,100,150,200,250)){
      plot <- plot_paired_read_eff(ylab = "accuracy",y = "acc",read_lengths = read_length,ylim = c(0.5,0.9),table = TRUE,nn_type = type)
...
for(type in c("cnn","lstm")){
  for(read_length in c(50,100,150,200,250)){
    plot <- plot_paired_read_eff(ylab = "auroc",y = "auroc",read_lengths = read_length,ylim = c(0.6,1),table = TRUE,nn_type = type)
...
for(type in c("cnn","lstm")){
  for(read_length in c(50,100,150,200,250)){
    plot <- plot_paired_read_eff(ylab = "precision",y = "precision",read_lengths = read_length,ylim = c(0.5,1),table = TRUE,nn_type = type)
...
for(type in c("cnn","lstm")){
  for(read_length in c(50,100,150,200,250)){
    plot <- plot_paired_read_eff(ylab = "recall",y = "recall",read_lengths = read_length,ylim = c(0.5,1),table = TRUE,nn_type = type)
...
for(type in c("cnn","lstm")){
  for(read_length in c(50,100,150,200,250)){
    plot <- plot_paired_read_eff(ylab = "spec",y = "spec",read_lengths = read_length,ylim = c(0,1),table = TRUE,nn_type = type)
