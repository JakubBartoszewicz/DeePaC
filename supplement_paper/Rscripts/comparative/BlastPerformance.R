...
Paired <- T
SetName <- "Tempo"
Path.L <- "SCRATCH_NOBAK/Benchmark_ROGAL/PathogenReads/OtherData/HP_NHP/testData/Reads/TempoLeft_250bp_fold1/Blast/"
...
for (trainingSet in c("AllTrainingGenomes", "AllStrains")){
    ...
    test.L <- get.performance(merged.L$MatchedLabel[!is.na(merged.L$MatchedLabel)], merged.L$QueryLabel[!is.na(merged.L$MatchedLabel)], P, N)
    
    if (Paired){
        test.R <- get.performance(merged.R$MatchedLabel[!is.na(merged.R$MatchedLabel)], merged.R$QueryLabel[!is.na(merged.R$MatchedLabel)], P, N)
