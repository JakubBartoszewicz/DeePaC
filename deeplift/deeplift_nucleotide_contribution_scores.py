...
def main():
	...
	RC_architecture = "RevCompConv1D" in [type(layer).__name__ for layer in deeplift_model.get_layers()]
	if RC_architecture:
		conv_layer_idx = [type(layer).__name__ for layer in deeplift_model.get_layers()].index("RevCompConv1D")
	...
	for filter_index in range(n_filters):
		...
		while i < num_reads_filter:
			...
			#build filter mask
			#filter = [[motif_start, filter_index] for motif_start in motif_starts[i:i+chunk_size]]
			filter =  np.stack((motif_starts[i:i+chunk_size], [filter_index]*num_reads), axis = 1)
			filter = np.repeat(filter, num_ref_seqs, 0)
...
def parse_arguments():
	...
	parser.add_argument("-r", "--ref_mode", default = "N", choices=['N', 'own_ref_file'], help="Modus to calculate reference sequences")
