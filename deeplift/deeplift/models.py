...
FuncType = deeplift.util.enum(
    ...
    multipliers_filter="multipliers_filter",
...
class Model(object):
    ...
    def get_target_multipliers_filter_func(self, *args, **kwargs):
        return self._get_func(*args, func_type=FuncType.multipliers_filter, **kwargs)
...
class SequentialModel(Model):
    ...
    def _get_func(self, find_scores_layer_idx,
                        target_layer_idx=-2, conv_layer_idx = -1, **kwargs):
        ...
        if (conv_layer_idx == -1 and "Conv1D" in [type(layer).__name__ for layer in self.get_layers()]):
            conv_layer_idx = [type(layer).__name__ for layer in self.get_layers()].index("Conv1D")
        return super(SequentialModel, self)._get_func(
                    find_scores_layers=find_scores_layers,
                    target_layer=self.get_layers()[target_layer_idx],
                    conv_layer=self.get_layers()[conv_layer_idx],
