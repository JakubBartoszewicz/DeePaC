...
class Conv1D(Conv):
    ...
    def _compute_shape(self, input_shape):
        ...
        if (input_shape is None or input_shape[1] is None):
            ...
            if (self.padding != PaddingMode.valid and self.padding != PaddingMode.same):
                raise RuntimeError("Please implement shape inference for"
                                   " border mode: "+str(self.padding))
            if (self.padding == PaddingMode.same):
                shape_to_return.append(int(input_shape[1]))
            if (self.padding == PaddingMode.valid):
                shape_to_return.append(1+int((input_shape[1]-self.kernel.shape[0])/self.stride))
